...
module.exports = function(parameters) {
  ...
  fontLoader.load('/demo/helvetiker_regular.typeface.json', font => {
    ...
    var textGeometry = new THREE.TextGeometry('CloudCastle', { font: font, size: 90, height: 10 });
    ...
    for (var i = 0; i < textGeometry.vertices.length; i++) {
      textGeometry.vertices[i].communicateObj =
        objects[THREE.Math.randInt(0, objects.length - 1)].position;
    }
    ...
    function toSpherePoints() {
      ...
      animateToSphere.forEach(function(tween, index) {
        tween
          ...
          .onComplete(function(data) {
            objectsAdded++;
            new TWEEN.Tween(objects[index].material)
              .to(
                {
                  opacity: 0,
                },
    ...
    function toTextPoints() {
      ...
      for (var i = 0; i < textGeometry.vertices.length; i++) {
        ...
        var tween = new TWEEN.Tween(vertice)
          .to(
            {
              x: textVertice.x,
              y: textVertice.y,
